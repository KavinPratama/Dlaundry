...
def loginadminD(data_adminD):
    ...
    if username == "" or password == "":
        print("Username dan password tidak boleh kosong. Ulangi login.")
...
def rekrut_staf():
    ...
    try:
        with open('data_staf.csv', 'a', newline='') as file:
            ...
            while True:
                umur_input = input("Masukkan umur (17-60): ").strip()
                
                if not umur_input.isdigit():
                    print("Umur harus berupa angka!")
                    input("Tekan Enter untuk mengisi ulang.")
                    continue

                umur = int(umur_input)

                if umur < 17:
                    print("Masih terlalu muda untuk bekerja.")
                    input("Tekan Enter untuk mengisi ulang.")
                elif umur > 60:
                    print("Sudah waktunya pensiun.")
                    input("Tekan Enter untuk mengisi ulang.")
                else:
                    break  
            ...
            if not notelp.isdigit() or not (12 <= len(notelp) <= 13):
    ...
    try:
        ...
        while True:
            try:
                ...
                if berat > 0.5 and int(berat) < 20:
                    break
                else:
                    print("Berat cucian harus lebih dari 0.5 kg dan maksimal 20 kg.")
            except ValueError:
    ...
    try:
        ...
        for row in daftar_pesanan:
            if row['username'].strip().lower() == username.lower() and row['Status'].strip().lower() != 'selesai':
                ...
                print("\nStatus baru: [1] Siap diambil, [2] Selesai")
                pilihan = input("Pilih status baru [1/2]: ").strip()
...
def cek_status_pesanan(username):
    ...
    try:
        with open('data_pesananD.csv', mode='r', newline='') as file:
            ...
            hasil = [
                row for row in reader 
                if row['username'] == username and row['Status'].lower() in ['diproses', 'diterima', 'siap diambil']
            ]
...
def cek_riwayat_pesanan(username):
    ...
    print("2. Urut berdasarkan BULAN (Jan - Des)")
    print("3. Urut berdasarkan TOTAL BIAYA (murah ke mahal)")
    print("4. Kembali ke halaman menu pelanggan")
    pilihan = input("Pilih metode pengurutan [1/2/3/4]: ")

    if pilihan == "4":
        return menu_pelanggan(username)
    ...
    try:
        ...
        if pilihan == "1":
            ...
            print("Pilihan tidak valid.")
            return cek_riwayat_pesanan(username)
